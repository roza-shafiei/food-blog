...
const recipeIdTagsLoader = document.querySelector('.recipe-id__tags__loader')
const recipeIdContentLoader = document.querySelector('.recipe-id__content__loader')
let loading = false
let tags = null
...
async function getRecipeId() {
    try {
        loading = true
        recipeIdContentLoader.style.display = 'block'
        const res = await fetch(`https://dummyjson.com/recipes/${id}`)
        const data = await res.json()
        if (data.id) {
            recipeIdContentLoader.style.display = 'none'
